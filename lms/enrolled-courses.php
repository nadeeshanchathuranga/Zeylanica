                ...
                <div class="course-card <?= $isExpired ? 'expired' : '' ?>" onclick="viewCourse('<?= $enrollment['course_id'] ?>')">
                    <div class="course-thumbnail">
                        <?php if ($enrollment['thumbnail_path']): ?>
                            <img src="<?= htmlspecialchars($enrollment['thumbnail_path']) ?>" alt="Course thumbnail">
                        <?php else: ?>
...
<style>
.enrolled-courses-container { max-width: 1200px; margin: 0 auto; }
...
.courses-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 2rem; }
...
.course-thumbnail { width: 100%; height: 200px; background: #f3f4f6; position: relative; }
...
.course-meta { display: flex; gap: 1rem; font-size: 0.875rem; color: #6b7280; margin-bottom: 1rem; }
...
</style>n-info { display: flex; gap: 1rem; flex: 1; }
.lesson-status i { font-size: 1.25rem; }
.lesson-status .completed { color: #10b981; }
.lesson-details h5 { font-size: 1rem; font-weight: 600; color: #111827; margin-bottom: 0.25rem; }
.lesson-meta { display: flex; gap: 1rem; font-size: 0.875rem; color: #6b7280; }
.free-badge { background: #3b82f6; color: white; padding: 0.125rem 0.5rem; border-radius: 4px; }
.last-watched { color: #10b981; }

.btn-watch { background: #3b82f6; color: white; padding: 0.5rem 1rem; border-radius: 6px; text-decoration: none; font-size: 0.875rem; display: flex; align-items: center; gap: 0.5rem; }
.btn-watch:hover { background: #2563eb; }
.btn-disabled { color: #9ca3af; font-size: 0.875rem; }

.btn { padding: 0.75rem 1.5rem; border: none; border-radius: 8px; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem; cursor: pointer; transition: all 0.2s; }
.btn-primary { background: #3b82f6; color: white; }
.btn-primary:hover { background: #2563eb; }

@media (max-width: 768px) {
    .course-header { flex-direction: column; gap: 1.5rem; text-align: center; }
...
echo renderAdminTemplate('My Learning Dashboard', $content);
?>